...
client = Groq(api_key=os.getenv("MISTRAL_API_KEY"))
...
def convert():
    ...
    if file and allowed_file(file.filename):
        ...
        lines = lines[1:]
        codee = '\n'.join(lines)
        if upload_to_s3(filename=f"{con_id}-old", filecontent=code) and upload_to_s3(filename=f"{con_id}-new", filecontent=codee):
...
def get_docs():
    ...
    chat_url = f"https://{S3_BUCKET_NAME}.s3.{AWS_DEFAULT_REGION}.amazonaws.com/{chatid}.json"
